 ...
 * 1. 합집합
 * 		union 연산	  : 중복을 제거
 * 		union all 연산 : 중복을 제거하지 않고, 원래 데이터 그대로 사용한다.
 * 	
 * 		A union B
 * 		A union all B
 * 
 ...
 *
 *		- 두 데이터 셋 모두 같은 수의 열을 가져야 한다.
 *		- 두 데이터 셋의 각 열의 자료형은 서로 동일해야 한다.
 *
 * 
 */
 ...
 * 
 * 1. 데이터 출처를 구분할 필요.
 * 2. 다중 복합 집합 연산시, 집합 연산 전 후의 count(*) 값을 비교해야한다. 
 * */

/* 결과가 798 rows 
 * 
 * 동명이인이 존재. => union all 을 사용해야한다.
 * 
 * */
