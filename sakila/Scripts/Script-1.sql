...
/* where 절 - 조건 절 */
/* file table : 출시되어 대여할 수 있는 DVD(영화)
 * 
 * rating : 등급
 ...
 * 등급이 PG-13 이면서, 단기 대여(4일 미만) 가능한 영화
 */
select title
from film
where(film.rating = 'G' and film.rental_duration >= 7) or 
(film.rating = 'PG-13' and rental_duration < 4);

/*
 * 고객별(그룹핑) 대여 횟수를 계산한 다음 대여 횟수가 40회 이상인 고객 리스트를 작성
 * 횟수 : count(*)
 */
 ...
 * 정렬 추가(오름차순) 고객명(last name, first name)
 */
select c.first_name , c.last_name , count(*)
	from customer c inner join rental r on c.customer_id = r.customer_id
	group by c.first_name , c.last_name /* 고객별 */
